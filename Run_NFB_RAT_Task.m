function FullTableRAT = Run_NFB_RAT_Task(windowPtr, folder, ID, textColor, bgColor, block, sessiontable, rect)
    PsychDefaultSetup(1); 
    ...
    PromptWordsList = transpose(sessiontable{block+1, 16:30});
 ...
 for i = 1:size(PromptWordsList, 1)
   ...
   % Screen('Flip', windowPtr);  % Flip to show the prompt word
    ...
    FileName = [folder, 'Sub', char(num2str(ID)), '_NFB', char(num2str(block-1)), '_RAT_Task_ResponseTable.xlsx'];
