function [string,terminatorChar] = GetEchoStringNFB(windowPtr, msg, x, y, textColor, bgColor, startTime, endTime, useKbCheck,varargin)
...
while GetSecs()-startTime < endTime
    if useKbCheck
        timeleft = endTime-(GetSecs()-startTime);
        char = GetKbChar([],timeleft);
    else
        char = GetCharNFB(startTime,endTime,0,0);
    end
