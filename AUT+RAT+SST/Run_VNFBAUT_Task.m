function FullTableAUT = Run_VNFBAUT_Task(windowPtr, ID,textColor,bgColor,block,sessiontable, rect)
...
%PromptWordsList = transpose(sessiontable{block+1,12:14});
PromptWordsList = transpose(sessiontable{block+1,13:15});
...
%           ' You will have 90 seconds to think of ideas \n...' ...
%           ' for each object in this session. \n\n'...
%           'Write down your ideas using the keyboard \n' ...
%           ' use the enter key to input your response. \n\n' ...
%           '      (Wait for the start)\n' ];
...
for i = 1:size(PromptWordsList,1) % height(PromptWordsList)%length(PromptWordsList)
    ...
    % Draw 'myText', centered in the display windowPtr:
        ...
        %ResponseTable.Response(CountResponses) = GetEchoStringNFB(windowPtr, 'Enter answer:', 25, ((rect(4) - rect(2)) / 2), textColor, bgColor, ExTime.StartPromptWord(i), Time2Res);
...
%for task add trial number 1-3 also to function (will use later
% FileName = [folder,'Sub',char(num2str(ID)),'_Session',num2str(sessiontable.Session(1)),'_Block',char(sessiontable.Trial(block+1)),'_AUTTask_ResponseTable'];
